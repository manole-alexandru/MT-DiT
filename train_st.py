...
"""
Single-task training script for DiT using the standard diffusion objective.
"""
import torch
...
def main(args):
    """
    Trains a new DiT model with the standard single-task diffusion objective.
    """
    ...
    for epoch in range(args.epochs):
        ...
        for x, y in progress:
            ...
            if train_steps % args.log_every == 0:
                ...
                avg_loss, avg_eps, avg_vb = avg_tensor.tolist()
                logger.info(f"(step={train_steps:07d}) Loss: {avg_loss:.4f} (eps={avg_eps:.4f}, vb={avg_vb:.4f}), Steps/Sec: {steps_per_sec:.2f}")
